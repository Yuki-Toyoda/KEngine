...
void Sprite::Update()
{
	...
	// 頂点をずらす
	plane_->vertices_.t[0].position = { left, top, 0.0f };  // 左下
	plane_->vertices_.t[1].position = { right, top, 0.0f };     // 左上
	plane_->vertices_.t[2].position = { right, bottom, 0.0f }; // 右下
	plane_->vertices_.t[3].position = { left, bottom, 0.0f };    // 右上
