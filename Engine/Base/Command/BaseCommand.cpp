...
void BaseCommand::Init(ID3D12Device* device, DXC* dxc, ID3D12RootSignature* signature, ID3D12Resource* resource, std::wstring vs, std::wstring ps, int32_t blendType)
{
	// PSOの生成
...
void BaseCommand::CreatePSO(ID3D12Device* device, DXC* dxc, ID3D12RootSignature* signature, std::wstring vs, std::wstring ps, int32_t blendType, UINT wire)
{
	// 基底クラスでは記述を行わない
	// 以下は警告回避の参照
	device;
