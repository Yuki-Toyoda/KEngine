...
void IParticleEmitter::GenerateParticle()
{
	...
	for (int i = 0; i < generateParticleCount_; i++) {
		...
		// 生成粒子の大きさ設定
		float size = Math::RandomF(1.5f, 2.0f, 1);
		Vector2 generateScale = { size, size };
		// 生成粒子の速度ベクトル設定
		Vector3 generateVelocity = { Math::RandomF(-0.1f, 0.1f, 3), Math::RandomF(-0.1f, 0.1f, 3), Math::RandomF(-0.1f, 0.1f, 3) };
		// 生成粒子の色
		...
		newParticle->PreInit(0.35f, transform_.translate_, generateScale, generateVelocity, texture_, generateColor);
